...
StartValues(const object){
	...
	// DO NOT REMOVE THESE 2 LINES WHEN USING THIS FUNCTION
	object.GetPara();  									
...
// SPECIFICATIONS:
	...
	garchobj.CSTS(i_csts1,i_csts2);			
		// cst in Mean (1 or 0) | cst in Variance (1 or 0)	|DEFAULT: 1, 1	
	...
	garchobj.ARMA_ORDERS(i_arma_orders1, i_arma_orders2); 													
		// AR order (p) | MA order (q) | DEFAULT: 0, 0		
	garchobj.ARFIMA(i_arfima);						
		// 1 if Arfima wanted | 0 otherwise | DEFAULT: 0		
	garchobj.GARCH_ORDERS(i_garch_orders1, i_garch_orders2);													
		// p order | q order | DEFAULT: 1, 1		
	garchobj.MODEL(i_model);					
		// 1 : GARCH | 2 : EGARCH | 3 : GJR | 4 : APARCH | 5 : IGARCH | 
	...
	garchobj.ARCH_in_mean(i_arch_in_mean);													
		// ARCH-in-mean | 1 add variance in cond. mean 
		// 2 add cond. std. | 0 otherwise | DEFAULT: 0			 				
	garchobj.TRUNC(i_trunc);				
		// Truncation order, only F.I. models with BBM method |DEFAULT: 100
	
// TESTS:

	garchobj.BOXPIERCE(<10; 15; 20>);				
		// Lags for the Box-Pierce Q-stats | <> otherwise		
	garchobj.ARCHLAGS(<2; 5; 10>);				
		// Lags for Engle's LM ARCH test | <> otherwise		
	garchobj.NYBLOM(1);							
		// 1 for Nyblom stability test | 0 otherwise  	    
	garchobj.PEARSON(<40;50;60>);			
		// Cells (<40;50;60>) for adjusted Pearson Chi-square 
	   	// Goodness-of-fit test | <> otherwise | G@RCH1.12	
	   		   	
// FORECASTS:
   	
//  garchobj.FORECAST(0, 10, 1);     				
		// Arg.1 : 1 launch forecasting procedure | 0 otherwise 
...
// OUTPUT:
	...
	garchobj.MLE(1);					
		// 0: both | 1: MLE | 2: QMLE	 	
	garchobj.COVAR(1);			    	
		// if 1, prints variance-covariance matrix | DEFAULT: 0		
	garchobj.ITER(5);					
		// Interval of iterations between printed intermediary results    
		// (if no intermediary results wanted, enter '0') | DEFAULT: 10	 	
	garchobj.TESTS(0, 0);				
		// Arg. 1 : 1 run tests PRIOR to estimation | 0 otherwise
		// Arg. 2 : 1 run tests AFTER estimation | 0 otherwise
...
// PARAMETERS:
	...
	garchobj.FixParam(0, <1; 0; 0; 0>); 
		...
		//   their starting values | 0 otherwise
		// Arg.2 : 1 to fix (see garchobj.DoEstimation(<>)) and 
		//   0 to estimate the corresponding parameter
...
// ESTIMATION:		
	...
	garchobj.DoEstimation() ;      	
		// m_vPar = m_clevel | m_vbetam |  m_dARFI | m_vAR | m_vMA | 
     	// m_calpha0 | m_vgammav | m_dD |  m_vbetav | m_valphav | 
       	// m_vleverage | m_vtheta1 | m_vtheta2 | m_vpsy | m_ddelta | 
   		// m_cA | m_cV | m_vHY | m_v_in_mean
	garchobj.Output();		
  
//  garchobj.STORE(1, 0, 1, 1, 1, "Ox", 0); 	                                
		//  Arg.1,2,3,4,5 :  if 1 -> stored. Names are
	   	//    (Res-SqRes-CondV-MeanFor-VarFor)
		//  Arg.6 :  Suffix. The name of the saved series will 
		//    be "Res_ARG6" (or "MeanFor_ARG6", ...).          	
...
// PRINT TO FILES:
	...
	fprint(file, garchobj.PAR() );					
	fclose(file);
	// Residuals
	file = fopen("OxResiduals.csv", "w"); 
	fprint(file, garchobj.GetValue("m_vE") );		
	fclose(file);
	// Conditional Variances
	file = fopen("OxCondVars.csv", "w"); 
	fprint(file, garchobj.GetValue("m_vSigma2") );	
	fclose(file);
