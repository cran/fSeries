...
# FUNCTION:                 UNIT ROOT TESTS
#   adfTest					  Augmented Dickey - Fuller Unit Root Test
#   ppTest					  Phillips - Perron Unit Root Test
#   kpssTest				  KPSS - Unit Root Test for Stationarity
# FUNCTION:                 COINTEGRATION TESTS:
#   poTest					  Phillips - Ouliaris Cointegration Test
################################################################################
...
tsTest = 
function(x, 
method = c("jb", "adf", "pp", "po", "kpss", "tnn", "wnn", "bds" ), ...)
{	
	# Load Library:
    # require(tseries)
    
    # As Vector:
    if (class(x) == "ts") x = as.vector(x)
	if (class(x) == "its") x = as.vector(x@.Data[, 1])
	
	# Settings:
	method = method[1] 
	test = paste(method, ".test", sep = "")
	if (method == "jb") test = "jarque.bera.test"
	if (method == "tnn") test = "terasvirta.test"
	if (method == "wnn") test = "white.test"
	fun = match.fun(test)
...
type = c("Z(alpha)", "Z(t_alpha)"), lshort = TRUE)
{
	...
	type = c("Z(alpha)", "Z(t_alpha)"), lshort = TRUE) {
	    ...
	    u = residuals(res)
	    ssqru = sum(u^2)/n
	    if (lshort) l = trunc(4*(n/100)^0.25)
...
function(x, demean = TRUE, lshort = TRUE)
{
	...
	function(x, demean = TRUE, lshort = TRUE) {
	    ...
	    if (demean) res = lm(x[,1]~x[,-1])
	    else res = lm(x[,1]~x[,-1]-1)
