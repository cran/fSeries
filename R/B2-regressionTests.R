...
{   # A function implemented by Diethelm Wuertz 
    ...
    # DW:
    if (method == "dw") ans = .dwtest(formula = formula, data = data, ...)

    # BP:
    if (method == "bp") ans = .bptest(formula = formula, data = data, ...)
    
    # GQ:
    if (method == "gq") ans = .gqtest(formula = formula, data = data, ...)
    
    # HMC:
    if (method == "hmc") ans = .hmctest(formula = formula, data = data, ...)
           
    # HARV:
    if (method == "harv") ans = .harvtest(formula = formula, data = data, ...)
       
    # RAIN:
    if (method == "rain") ans = .raintest(formula = formula, data = data, ...)
    
    # RESET:
    if (method == "reset") ans = .reset(formula = formula, data = data, ...)
          
    # BG:
    if (method == "bg") ans = .bgtest(formula = formula, data = data, ...)
