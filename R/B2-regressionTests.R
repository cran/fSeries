...
{   # A function implemented by Diethelm Wuertz 
    ...
    # DW:
    if (method == "dw") {
    BIdwtest = function(formula, alternative = c("greater", "two.sided", 
    "less"), iterations = 15, exact = NULL, tol = 1e-10, data = list()){
        ...
        if(exact){
            ...
            if ((pval > 1) | (pval < 0)) {
                ...
                warning("approximate p value will be used.")
                exact = FALSE } }
        if (!exact) {
            XAXQ = t(X) %*% A %*% X %*% Q1
            P = 2*(n-1) - sum(diag(XAXQ))
            Q = 2*(3*n - 4) - 2* sum(diag(t(X) %*% A %*% A %*% X %*% Q1)) + 
                sum(diag(XAXQ %*% XAXQ))
            dmean = P/(n-k)
            dvar = 2/((n-k)*(n-k+2)) * (Q - P*dmean)
            pval = switch(alternative,
                "two.sided" = (2*pnorm(abs(dw-dmean), 
                    sd  =sqrt(dvar), lower.tail = FALSE)),
                "less" = pnorm(dw, mean = dmean, 
                    sd = sqrt(dvar), lower.tail = FALSE),
                "greater" = pnorm(dw, mean = dmean, 
                    sd = sqrt(dvar))) } 
        ...
        return(RVAL) }
    # Return value:
    return(BIdwtest(formula = formula, ...)) }

    
    # BP:
    if (method == "bp") {
    BIbptest = function(formula, varformula = NULL, studentize = TRUE,
    data = list()){
        dname = paste(deparse(substitute(formula)))
        mf = model.frame(formula, data = data)
        y = model.response(mf)
        ...
        return(RVAL) }
    # Return value:
    return(BIbptest(formula = formula, ...)) }
    
    # GQ:
    if (method == "gq") {
    BIgqtest = function(formula, point=0.5, order.by=NULL, data=list()) {
        dname = paste(deparse(substitute(formula)))
        mf = model.frame(formula, data = data)
        y = model.response(mf)
        X = model.matrix(formula, data = data)
        ...
        return(RVAL) }
    # Return value:
    return(BIgqtest(formula = formula, ...)) }
    
    
    # HMC:
    if (method == "hmc") {
    BIhmctest = function(formula, point = 0.5, order.by = NULL, 
    simulate.p = TRUE, nsim = 1000, doplot = FALSE, data = list()) {
        dname = paste(deparse(substitute(formula)))
        mf = model.frame(formula, data = data)
        y = model.response(mf)
        X = model.matrix(formula, data = data)
        ...
        return(RVAL)}
    # Return value:
    return(BIhmctest(formula = formula, ...)) }
    
        
    # HARV:
    if (method == "harv") {
    BIharvtest = function(formula, order.by = NULL, data = list()){
        dname = paste(deparse(substitute(formula)))
        mf = model.frame(formula, data = data)
        y = model.response(mf)
        X = model.matrix(formula, data = data)
        ...
        return(RVAL)}
    # Return value:
    return(BIharvtest(formula = formula, ...)) }
    
        
    # RAIN:
    if (method == "rain") {
    BIraintest = function(formula, fraction = 0.5, order.by = NULL, 
    center = NULL, data=list()) {
        dname = paste(deparse(substitute(formula)))
        mf = model.frame(formula, data = data)
        y = model.response(mf)
        X = model.matrix(formula, data = data)
        ...
        if (is.null(order.by)){
            ...
            if(!is.null(class(order.by)) && class(order.by)=="formula"){
                ...
                o = order(mahalanobis(X,center, chol2inv(qr.R(qr(X))), 
                    inverted = TRUE))
                X = as.matrix(X[o,])
                y = y[o]
                from = 1
        ...
        return(RVAL) }
    # Return value:
    return(BIraintest(formula = formula, ...)) }
    
    # RESET:
    if (method == "reset") {
    BIresettest = function(formula, power = 2:3, type = 
    c("fitted", "regressor", "princomp"), data = list()) {
        dname = paste(deparse(substitute(formula)))
        mf = model.frame(formula, data = data)
        y = model.response(mf)
        X = model.matrix(formula, data = data)
        ...
        "regressor" = {
            ...
            Z = matrix(as.vector(t(sapply(as.vector(Z), "^", power))), 
                nrow=n) },
        "princomp" = {
            Z = as.matrix(mf[,which(!sapply(mf,is.factor))[-1]])
        ...
        return(RVAL) }
    # Return value:
    return(BIresettest(formula = formula, ...)) }
    
        
    # BG:
    if (method == "bg") {
    BIbgtest = function(formula, order = 1, type = c("Chisq", "F"), 
    data = list()) {
        dname = paste(deparse(substitute(formula)))
        mf = model.frame(formula, data = data)
        y = model.response(mf)
        ...
        RVAL = list(statistic = bg, parameter = df,
            method = 
            paste("Breusch-Godfrey test for serial correlation of order", 
                max(order)),
            p.value = p.val,
            data.name =   dname)
        class(RVAL) = "htest"
        return(RVAL) }
    # Return value:
    return(BIbgtest(formula = formula, ...)) }
